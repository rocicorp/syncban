...
export default function Electric() {
  ...
  const removeTask = async (task: DeleteTaskRequest) => {
    await fetchAndWaitForShape(
  ...
  const { mutateAsync: removeItemMut } = useMutation({
    ...
    mutationFn: (task: DeleteTaskRequest) => removeTask(task),
  ...
  pendingAdds.forEach((req) => {
    ...
    const existing = items.findIndex((task) => task.id === req.id);
    if (existing !== -1) {
      items.splice(existing, 1);
    }

    const destSiblings = items.filter((task) => task.columnID === req.columnID);
    destSiblings.sort(compareOrdered);

    const order = generateKeyBetween(
      destSiblings[destSiblings.length - 1]?.order ?? null,
      null
    );
